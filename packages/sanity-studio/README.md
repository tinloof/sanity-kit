...
- `keepQueryParameters` (boolean, optional, default false): **Keep query parameters** copies incoming query values when the matched redirect opts in. Destination query keys take priority; repeated incoming values for other keys are preserved by `@tinloof/sanity-next`. Existing rules without this field retain their behavior.
