...
- [Schema utilities](#schema-utilities)
  ...
  - [`defineDocument`](#definedocument)
    - [Field Overrides](#field-overrides-1)
...
export default definePage({
  ...
  fieldOverrides: {
    pathname: false, // Remove pathname field entirely
    seo: (field) => ({
      ...field,
      title: "Custom SEO Settings",
      hidden: false,
    }),
  },
  fields: [
...
The `fieldOverrides` option allows you to customize or remove the default fields that `definePage` adds automatically. You can override the following default fields:

- `pathname` - The pathname field for URL routing
- `seo` - The SEO settings object field

Each field override can be:

- `false` - Removes the field entirely
- A function `(field) => modifiedField` - Transforms the field
- A partial object - Shallow merges with the field
...
export default definePage({
  ...
  type: "document",
  fieldOverrides: {
    // Remove the pathname field
    pathname: false,

    // Customize the SEO field
    seo: {
      title: "Page SEO",
    ...
    // Or use a function for complex transformations
    seo: (field) => ({
      ...field,
      title: "Advanced SEO",
      fields: [
...
export default defineDocument({
  ...
  fieldOverrides: {
    internalTitle: false, // Remove internal title field
    locale: (field) => ({...field, hidden: false}),
    orderRank: {hidden: false},
  },
  fields: [
    // Your custom fields
  ],
...
The `fieldOverrides` option allows you to customize or remove the default fields that `defineDocument` adds automatically. You can override the following default fields:

- `orderRank` - The order ranking field (when `orderable: true`)
- `locale` - The locale field (when `localized: true`)
- `internalTitle` - The internal title field

Each field override works the same way as in `definePage`:
...
export default defineDocument({
  ...
  fieldOverrides: {
    // Remove the order rank field
    orderRank: false,

    // Make locale field visible for debugging
    locale: {hidden: false},

    // Customize internal title
    internalTitle: {
      title: "Post Name",
      description: "Internal name for this post",
    },
  },
  fields: [
    // Your custom fields
...
A utility function for applying field overrides to an array of Sanity fields. This is the underlying function used by `definePage` and `defineDocument` for their `fieldOverrides` functionality, but can also be used independently for custom field manipulation.

#### Basic usage

```tsx
import {applyFieldOverrides, defineField} from "@tinloof/sanity-studio";
...
const modifiedFields = applyFieldOverrides(fields, {
...
- **fields**: `FieldType[]` - Array of fields created via `defineField`
- **overrides**: `Record<string, FieldOverride>` - Object where each key corresponds to a field name and each value can be:
  - `false` to remove the field
  - A function `(field) => modifiedField` to transform it
  - A partial object to shallow-merge into the field

#### Return value

Returns the transformed field array with falsy values filtered out.

#### Advanced usage

```tsx
/**
 * Custom schema helper that adds standard fields with overrides
 */
function defineCustomSchema(config) {
  ...
  const allFields = [...standardFields, ...config.fields];

  return {
    ...config,
    fields: config.fieldOverrides
      ? applyFieldOverrides(allFields, config.fieldOverrides)
      : allFields,
  };
}

// Usage
export default defineCustomSchema({
  name: "article",
  title: "Article",
  type: "document",
  fieldOverrides: {
...
Here are some practical examples of using field overrides in real-world scenarios:

#### Customizing SEO Fields

```tsx
// Add custom SEO fields for e-commerce pages
export default definePage({
  name: "productPage",
  title: "Product Page",
  fieldOverrides: {
    seo: (field) => ({
      ...
      fields: [
        ...field.fields,
        {
          ...
          type: "text",
          description: "JSON-LD structured data for products",
        },
        {
          name: "availability",
...
```tsx
// Make internal title required and customize locale field
export default defineDocument({
  name: "blogPost",
  title: "Blog Post",
  type: "document",
  ...
  fieldOverrides: {
    ...
    locale: {
      hidden: false, // Show locale field for debugging
      readOnly: true,
    },
    orderRank: (field) => ({
      ...field,
      title: "Post Order",
      description: "Drag to reorder posts in the list",
    }),
  },
  fields: [
    {
...
```tsx
// Advanced field override with conditional logic
export default definePage({
  name: "landingPage",
  title: "Landing Page",
  type: "document",
  fieldOverrides: {
    seo: (field) => {
      // Add A/B testing fields to SEO
      const originalFields = field.fields || [];
    ...
    pathname: (field) => ({
      ...
      validation: (Rule) =>
        Rule.required().custom((value) => {
          if (value?.current?.includes("/landing/")) {
            return true;
          }
          return 'Landing pages must have "/landing/" in their URL';
        }),
    }),
  },
  fields: [
...
#### Using applyFieldOverrides Independently

```tsx
import {applyFieldOverrides, defineField} from "@tinloof/sanity-studio";

// Create a reusable field configuration function
function createStandardFields(overrides = {}) {
  const baseFields = [
    defineField({
      name: "createdAt",
      type: "datetime",
  ...
  return applyFieldOverrides(baseFields, overrides);
}

// Use in different schemas
export const articleSchema = {
  name: "article",
  type: "document",
  fields: [
    ...createStandardFields({
      status: {initialValue: "draft"},
      createdAt: false, // Remove for articles
    }),
    defineField({
      name: "title",
      type: "string",
