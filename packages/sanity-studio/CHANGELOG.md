...
- ee37944: Paginate the redirects editor in groups of 50, with full-list search, result counts, and previous/next controls. Preserve active entries, validation errors, and original array indexes while paging.
...
- 379b567: Add the opt-in `@tinloof/sanity-studio/table` entry point with `defineTableSchema` and `withTableMerging`. Supports horizontal cell merge/split, logical column editing, keyed edits, and configurable rich clipboard rendering while preserving the table/rows/cells/value data shape. Existing Portable Text fields are unchanged. Table integration requires `@portabletext/editor` 8.1.3 and is verified with Sanity 6.13.1.

## 3.0.0

### Major Changes

- 94e9e1a: Require Sanity 6.12 or later within major 6 (`^6.12.0`) and drop Sanity 5 support. Update Studio integrations for Sanity UI 4, Sanity icons 5, and React 19.2.4 or later within major 19. Require Node.js 22.12 or later.
...
- 6a563a4: Move pathname field to top of page abstract and collapse SEO object by default
