...
The first matching array entry wins within each lookup. Query-specific rules take priority over path rules regardless of array order. Incoming query parameters are not appended to the destination. Destination query strings and fragments are preserved through URL resolution. Permanent redirects return **301** and temporary redirects return **302**. Every lookup uses `perspective: "published"` and `stega: false`. An unmatched query adds one lookup before path fallback; requests without a query still need only one.
...
A custom query receives `$paths: string[]` and must return `{source: string, destination: string, permanent: boolean}` or `null`. The same query runs for each lookup: query-bearing path variations first, then path-only variations if necessary. Existing `$paths` queries need no additional parameters.
