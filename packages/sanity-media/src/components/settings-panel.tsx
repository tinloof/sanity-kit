...
export function SettingsPanel({adapter, onBack}: SettingsPanelProps) {
	// Presigned mode: no credential fields to configure
						...
						<Card padding={5} radius={2} border tone="positive">
							<Flex align="center" gap={3}>
								...
								<Stack space={2}>
									...
									<Text size={1} muted>
										Uploads are handled server-side. No credentials needed in
										the browser.
									</Text>
								</Stack>
							</Flex>
	...
	// Credential-based mode: render the form
				...
				<Box paddingX={4}>
					<Flex justify="space-between" align="flex-start">
						<Stack space={2}>
							...
							<Text size={1} muted>
								Configure your {adapter.name} credentials to enable media
								uploads
							</Text>
						</Stack>
						{onBack && (
