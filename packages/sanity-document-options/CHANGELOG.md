...
- 129db91: Add document-level `options.structureOptions: false` to omit a type from autogenerated structure without removing its schema, contextual lists, references, templates, or creation options. Existing singleton restrictions still apply, and groups containing only hidden types are omitted.
